...
from .presets import ARCHETYPES, GUI_FRAMEWORKS, ADDONS, DEV_BUNDLES # type: ignore
...
def collect_config() -> ProjectConfig:
    ...
    section("Archetype")
    archetype = ask_choice("Select archetype", ARCHETYPES, default_index=0)
    ...
    if archetype == "gui":
        ...
        gui_labels = [label for (label, _pkgs) in GUI_FRAMEWORKS] # type: ignore
        gui_fw = ask_choice("Select GUI framework", gui_labels, default_index=0) # type: ignore

        fw_map = {label: pkgs for (label, pkgs) in GUI_FRAMEWORKS} # type: ignore
        extra_pkgs += fw_map.get(gui_fw, []) # type: ignore
    ...
    # Dev tools bundle
    section("Dev Tools")
    dev_bundle_labels = [label for (key, label, _pkgs, _default) in DEV_BUNDLES] # type: ignore
    ...
    deps, _ = resolve_deps(Selection(archetype, gui_fw, addon_keys, dev_bundle_key)) # type: ignore
    deps += extra_pkgs # type: ignore
    deps = list(dict.fromkeys(deps))  # type: ignore # de-dupe stable order
    ...
    return ProjectConfig(
        ...
        archetype=archetype,  # must match your Archetype Literal # type: ignore
        use_src_layout=use_src,
        create_venv=create_venv,
        init_git=init_git,
        self_destruct=self_destruct,
        extra_packages=deps,
        dev_packages=dev_pkgs,  # type: ignore
    )
