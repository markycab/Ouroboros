...
# Document Generation
if prompt_yes_no "Do you need document generation capabilities?" "n"; then
    echo -e "${BLUE}  ✓ Document generation selected${NC}"
    
    if prompt_yes_no "  Install Jinja2 (template engine)?" "y"; then
        SELECTED_PACKAGES+=("jinja2")
        echo -e "${GREEN}  → Jinja2 will be installed${NC}"
    fi
    
    if prompt_yes_no "  Install python-docx (Word documents)?" "n"; then
        SELECTED_PACKAGES+=("python-docx")
        echo -e "${GREEN}  → python-docx will be installed${NC}"
    fi
    
    if prompt_yes_no "  Install reportlab (PDF generation)?" "n"; then
        SELECTED_PACKAGES+=("reportlab")
        echo -e "${GREEN}  → reportlab will be installed${NC}"
    fi
    
    if prompt_yes_no "  Install weasyprint (HTML to PDF)?" "n"; then
        SELECTED_PACKAGES+=("weasyprint")
        echo -e "${GREEN}  → weasyprint will be installed${NC}"
        echo -e "${YELLOW}  ⚠ Note: weasyprint requires system dependencies (Cairo, Pango)${NC}"
    fi
    
    if prompt_yes_no "  Install markdown (Markdown to HTML)?" "n"; then
        SELECTED_PACKAGES+=("markdown")
        echo -e "${GREEN}  → markdown will be installed${NC}"
    fi
fi
...
# Create templates directory examples if Jinja2 is selected
if [[ " ${SELECTED_PACKAGES[@]} " =~ " jinja2 " ]]; then
    mkdir -p templates/reports
    mkdir -p templates/emails
    
    # Create example Jinja2 template
    cat > templates/reports/example_report.html << 'EOF'
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body>
    ...
    <footer>
        <p><em>Generated by {{ app_name }}</em></p>
    </footer>
</body>
</html>
EOF

    # Create example template renderer utility
    cat > app/utils/template_renderer.py << 'EOF'
"""
...
def render_template(template_name: str, context: dict) -> str:
    ...
    Args:
        template_name: Name of the template file (e.g., 'reports/example_report.html')
    ...
    Example:
        ...
        ...     ]
        ... }
        >>> html = render_template('reports/example_report.html', context)
    """
...
def save_rendered_template(template_name: str, context: dict, output_path: str):
    """
...
#     'items': [
#         {'name': 'Sales', 'value': '$10,000', 'status': 'Complete'},
#         {'name': 'Expenses', 'value': '$5,000', 'status': 'Complete'},
#     ]
# }
# html_output = render_template('reports/example_report.html', context)
EOF
...
This directory contains Jinja2 templates for document generation.

## Structure

- `reports/` - Report templates (HTML, text, etc.)
...
```python
from app.utils.template_renderer import save_rendered_template
...
html = render_template('reports/example_report.html', context)
HTML(string=html).write_pdf('output/report.pdf')
```

## Template Syntax

Jinja2 uses the following syntax:
